...
namespace MLAPI
{
    ...
    public sealed class NetworkedObject : MonoBehaviour
    {
        ...
        internal NetworkedBehaviour GetBehaviourAtOrderIndex(ushort index)
        {
            ...
            {
                if (NetworkLog.CurrentLogLevel <= LogLevel.Error) NetworkLog.LogError("Behaviour index (" + index +") was out of bounds (" + childNetworkedBehaviours.Count + "). Did you mess up the order of your NetworkedBehaviours?");
                return null;
            }
